 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of 4 numbers`: Unit = {
    assert(sum(List(-1, 5, 10 , 90)) == 104)
  }
  ...
  @Test def `max of an empty list`: Unit = {
    assert(max(List()) == 0)
  }

  @Test def `max of a few numbers 4numbers`: Unit = {
    assert(max(List(-1,2,789,-888)) == 789)
  }

  @Test def `max of 4 numbers with negatives`: Unit = {
    assert(max(List(-1,2,-789,-888)) == 2)
  }

  @Test def exceptionTesting(): Unit = {
    intercept[NoSuchElementException] { // Result type: NoSuchElementException
      max(List())
    }
  }
